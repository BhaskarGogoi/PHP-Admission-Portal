		...
		if (in_array($fileActualExt, $allowed)) {
			if ($fileError === 0) {
				if ($fileSize < 100000) {
					...
					$fileLocation = $_SERVER['DOCUMENT_ROOT'].'/ccs-dr/admission/img/applicant-image/'.$fileNameNew;
					move_uploaded_file($fileTmpName, $fileLocation);
					$sql = "INSERT INTO applicant_image (applicant_id, application_id, applicant_image_status)
						VALUES ('$session', '$app_id', 1)";
					$result = $conn->query($sql);

					header("Location: //localhost/ccs-dr/admission/applicant-marks?step-two=success");
